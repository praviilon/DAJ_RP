...
void NPC_BSGrenadier_Attack( void )
{
	...
	if (TIMER_Done( NPCS.NPC, "zyk_check_enemy" ))
	{
		...
		{ // zyk: if enemy cant be seen, try getting one later
			// GalaxyRP fix: [Guardian] removed an always-false guardian_mode>0 quest-guardian enemy-acquisition branch here — guardian_mode is permanently 0 (spawn_boss has no callers)
			if (NPCS.NPC->client)
			{
				NPCS.NPC->enemy = NULL;
				NPC_BSGrenadier_Patrol();
