...
void NPC_BSSniper_Attack( void )
{
	...
	if (TIMER_Done( NPCS.NPC, "zyk_check_enemy" ))
	{
		...
		{ // zyk: if enemy cant be seen, try getting one later
			// GalaxyRP fix: [Guardian] removed a dead "guardians find enemies via quest player/allies" branch here — guardian_mode is permanently 0 (spawn_boss has no callers)
			if (NPCS.NPC->client)
			{
				NPCS.NPC->enemy = NULL;
				NPC_BSSniper_Patrol();
