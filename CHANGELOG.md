...
- `/engage_fullforceduel` command: a full force private duel, alongside the ordinary saber one. Both players must ask for this kind before it starts, force powers only work on your own opponent, and the challenge and acceptance messages say which kind of duel is on offer. Gated by `rp_allow_force_duel` (default 1), with `g_forceDuelForceRegenTime` and `g_saberDuelForceRegenTime` (both default 200) to tune force regeneration inside each kind of duel — the shipped server config sets both to 60 to match its own `g_forceRegenTime`. Adapted from TaystJK. `/list commands` documents both duel commands under Misc, where neither had appeared before.
